// Radiant Shine Wash — e-sign setup, headless (LIVE CRM/Supabase).
//
// Produces a client signing link WITHOUT the CRM emailing Mark, so Jeff can send
// it from his own inbox with his own context. Steps:
...
//
// AUTO PAYMENT-LINK SUPPRESSION: when Mark completes signing, the /sign route
// auto-creates a setup-fee payment link (from account.setup_fee_cents) and emails
// it with the executed copy. That $2,000-only link would CONTRADICT the combined
// $3,298 first-payment link we already built. So this script zeroes
// account.setup_fee_cents (setup collection lives on the manual combined link; the
// $2,000 figure is preserved on the deal row + account notes). On completion Mark
// still gets his executed copy by email, but no conflicting payment link.
//
...
//
// Usage:  node scripts/radiantshine-sign-setup.mjs /abs/path/to/agreement.pdf
...
const CONTRACT_TITLE = "SEO & Google Business Profile Services Agreement";
const PROVIDER_ORG = "JMC Companies LLC";
const PROVIDER_SIGNER = { name: "Jeff Conn", title: "Managing Member", email: "jconn333@gmail.com" };
const CLIENT_SIGNER = { name: "Mark Miller", email: "homecustomexteriors@yahoo.com" };
...
async function main() {
  ...
  if (!contract) {
    ...
    const ins = await db.from("contracts").insert({
      ...
      description: "Effective Aug 29, 2026. Radiant Shine Wash, LLC. SEO + GBP agents. Counter-signed by JMC; client link delivered by Jeff (not the CRM).",
